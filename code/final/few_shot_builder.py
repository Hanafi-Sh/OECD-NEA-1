...
few_shot_builder.py
-------------------
Dynamically builds Few-Shot JSON prompts for the LLM by analyzing the correlation 
between the raw extracted text (Inputs 1, 2, 3) and the golden final values (Outputs 1, 2, 3).
This ensures the AI learns the exact reasoning patterns required for the competition.
"""
...
def get_few_shots_for_column(col_name):
    """
    Constructs a unified JSON string containing all example pairs (Input 1, 2, 3)
    specifically tailored for extracting the target `col_name`.
    
    Using JSON arrays ensures structural integrity against special characters 
    present in the raw text.
    """
    examples_by_col = _load_all_data()
    
    if col_name not in examples_by_col:
        return "[]" # Return empty JSON array if column mapping is isolated
        
    examples_list = examples_by_col[col_name]
    
    # Dump to JSON string.
    # ensure_ascii=False preserves human-readable characters.
    # indent=None compresses the string to minimize token footprint.
